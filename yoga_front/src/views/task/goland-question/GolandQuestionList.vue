<template>
    <b-row>
        <b-col>
            <div class="mb-4">
                <b-button :to="{name: 'goland-question-create', params: {taskId: taskId}}" variant="primary" size="md">
                    Добавить
                </b-button>
                &nbsp;
            ...
            >
                ...
                <template v-slot:cell(actions)="data">
                    <div class="table__actions">
                        <b-button class="btn_edit" :to="{name: 'goland-question-update', params: {id: data.item.id, taskId: taskId}}"></b-button>
                        <!--<btn-turn :turn="true"/>-->
                        <b-button class="btn_delete" @click="deleteGolandQuestion(data.item.id)"/>
                    </div>
                </template>
            </b-table>
        </b-col>
...
export default {
    name: "GolandQuestionList",
    mixins: [GolandQuestionMixin],
    data () {
        return {
            taskId: null,
            fields: [
    ...
    created() {
        ...
        this.getGolandQuestions()
    },
    methods: {
        goFilter(e) {
            e.preventDefault();
            this.golandQuestionSearch = this.filter
            this.getGolandQuestions()
        },
        clearFilter() {
            this.filter = {}
            this.golandQuestionSearch = {}
            this.getGolandQuestions()
        },
    }
}
